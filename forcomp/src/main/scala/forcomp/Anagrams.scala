...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.groupBy(i => i).map { case (c, s) => (c, s.length) }.toList.sorted
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    loadDictionary.foldLeft(Map[Occurrences, List[Word]]() withDefaultValue List())(
      (acc, w) => {
  ...
  //          if (count == 0) iter(tail, acc) else iter((char, count - 1) :: tail, List((char, count)) :: acc)
  //        case Nil => acc
  //      }
  //    }
  //
